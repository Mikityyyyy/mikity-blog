...
`public/journal-cover-training.webp` はブログ紹介用のAI生成イメージです。運動施設のベンチにシューズ・バッグ・ノートを配した構成です。Mikity本人の所有物や実際の体験を撮影した写真ではありません。記事写真が利用できる場合はそちらを優先します。
...
サンプルの画像は内蔵ImageGenで作成したイメージです。`sample-running.webp` は朝の川沿いの道、`sample-strength.webp` はバーベルとタオル、`journal-cover.webp` は机・ノート・PCを配した写真風の構成です。共通の生成指示は、自然光、青灰色と生成り、人物・文字・ロゴなし。
