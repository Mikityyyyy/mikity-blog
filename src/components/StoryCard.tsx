...
export default function StoryCard({ post }: { post: BlogPost }) {
  ...
  return <article className="story-card"><Link href={`/blog/${post.slug.current}`}>
    <div className="story-image">{image ? <Image src={image} alt={post.mainImage?.alt || post.title} fill sizes="(max-width: 640px) 100vw, 33vw" /> : <div className="story-no-image" aria-hidden="true">mikitylife.</div>}</div>
